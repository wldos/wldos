...
public interface DomainResourceRepo extends PagingAndSortingRepository<WoDomainResource, Long> {
	...
	@Query("select s.*, r.id domain_id, r.module_name, r.term_type_id from wo_domain_resource r join wo_resource s on r.app_id=s.app_id and r.resource_id = s.id "
			+ "where r.domain_id=:domainId and r.is_valid = '1' and r.delete_flag = 'normal' and r.module_name != 'static' "
			+ "and s.is_valid='1' and s.delete_flag='normal'")
