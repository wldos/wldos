...
import com.wldos.system.sysenum.ResourceEnum;
...
public class AuthService {
	...
	public AuthVerify verifyReqAuth(String appCode, Long userId, String reqUri, String reqMethod) {
		...
		List<AuthInfo> matchAuthInfos = // 匹配请求URI的相关权限
				allMenuAuthInfo.parallelStream().filter(authInfo -> {
					...
					if (uri.indexOf("{") > 0) { // Redis cache json
	...
	public List<AuthInfo> queryAllAuth(String appCode) {
		...
		String key = RedisKeyEnum.wldosAuth.toString() + ":" + appCode;
		...
		if (ObjectUtil.isBlank(value)) {
			...
			Iterable<AuthInfo> authInfos = this.resourceService.queryAuthInfo(appCode);
			value = new Gson().toJson((List<AuthInfo>) authInfos);

			this.cache.set(key, value, 12, TimeUnit.HOURS);
		}

		List<AuthInfo> authInfos = JSON.parseArray(value, AuthInfo.class);
		return authInfos;
	}
	...
	public List<Menu> queryAdminMenuByUserId(Long curUserId) {
		List<WoResource> resources = this.resourceService.queryResource(ResourceEnum.adminMenu.toString(), curUserId);
		List<Menu> menus = resources.parallelStream().map(res -> {
			// 取出菜单
			Menu node = new Menu();
			node.setPath(res.getResourcePath());
	...
	public List<String> queryAuthorityButton(Long id) {
		...
		List<WoResource> resources = (isGuest = this.isGuest(id)) ? this.resourceService.queryResourceForGuest(ResourceEnum.button.toString())
