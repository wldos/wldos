...
/**
 * AES加/解密工具类。使用密钥时请使用 initKey() 方法来生成随机密钥，initKey方法内部使用 java.crypto.KeyGenerator密钥生成器来生成特定于AES
 * 算法参数集的随机密钥，使用密钥生成器生成的密钥能保证更强的随机性。生成的二进制密钥建议使用 Hex 进行编码。
 */
public class AesEncryptUtils {
    ...
    public static void main(String[] args) throws Exception {
       ...
       // byte[] bytes = AesEncryptUtils.initKey();
		...
		String username = "wldos";
		String aesKey = enCryHexKey(username);
        System.out.println(aesKey);

        //String aesKey = "d86d7bab3d6ac01ad9dc6a897652f2d2";

        String password = "sadfklsdf213424334";
        System.out.println("加密前：" + password);

        String encrypt = aesEncrypt(password, aesKey);
	 ...
	 * @throws Exception
	 */
    private static String enCryHexKey(String username) throws Exception {
		Date now = new Date();
		String date = DateUtil.getDay(now);
		String firstName = username.charAt(0) + "";
		String secondName = username.charAt(1) + "";
		String key = firstName + "ZHILETUWLDOS" + secondName + date;
		String keyHex = Hex.encodeHexString(key.getBytes(StandardCharsets.UTF_8));
