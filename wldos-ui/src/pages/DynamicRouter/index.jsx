...
// 插件组件加载器
const PluginComponentLoader = ({ pluginCode, component, menu, pluginManifest, ...props }) => {
	...
	const loadPluginComponent = async (code, compName, manifest) => {
		try {
			...
			if (process.env.NODE_ENV === 'development') {
				console.log(`[DynamicRouter] 开始加载插件组件: ${code}, 组件名: ${compName}`);
			}
			...
			try {
				...
				if (typeof window !== 'undefined') {
					...
					if (process.env.NODE_ENV === 'development') {
						console.log(`[DynamicRouter] 插件 request 已注入: ${code}`);
					}
				}
			} catch (requestError) {
				// request 注入失败不影响组件加载，只记录警告
				if (process.env.NODE_ENV === 'development') {
					console.warn(`[DynamicRouter] 插件 request 注入失败: ${code}`, requestError);
				}
			}
			...
			if (process.env.NODE_ENV === 'development') {
				try {
					...
					if (isPluginInDevMode(code)) {
						...
						console.log(`[DynamicRouter] 开发模式加载成功: ${code}`, { component: compName });
					...
					if (process.env.NODE_ENV === 'development') {
						console.warn(`[DynamicRouter] 开发模式加载失败，回退到正常加载: ${code}`, devError);
					}
				}
			}

			// 从manifest中获取插件信息（同部署方案，统一使用ESM）
			...
			// 控制台去噪：仅在开发环境打印详细错误
			if (process.env.NODE_ENV === 'development') {
				console.error(`[DynamicRouter] 插件加载失败: ${code}`, err);
			}
	...
	if (error) {
		return (
			<GridContent>
				<Card>
					<Alert type="error" message="插件加载失败" description={error} />
				</Card>
			</GridContent>
		);
	}
...
const DynamicRouter = (props) => {
  const { dynamicRoutes, pluginManifest, dispatch } = props;
	...
	// 如果 DynamicRouter 被渲染，说明没有静态路由匹配，无需再次检查
	...
	if (_match.component) {
		...
		const componentPath = getComponentPath(_match, pluginManifest);
		...
		// 判断是插件路径还是本地组件路径
		// 插件路径：通过 _match.type 判断，getComponentPath 返回的是相对路径（如 "airdrop/1.0.0/dynamic-airdrop-Execution.js"）
		// 本地组件：getComponentPath 返回的是中间部分（如 "sys/plugins"）
		let LazyComp;
		const isPluginRoute = _match.type === 'plugin_menu';

		if (isPluginRoute) {
			// 插件路径：从 _match.path 提取 pluginCode，从 manifest 获取版本信息
			// componentPath 格式：规范化路径（如 "tasks", "scheduler"）
			// 需要拼装为：/plugin-assets/{pluginCode}/{version}/dynamic-{componentPath}-index.js

			// 从 _match.path 提取 pluginCode
			const pathParts = _match.path.split('/').filter(part => part.length > 0);
			let pluginCode = null;
								...
								setTimeout(() => {
									if (!moduleResolved) {
										try {
											...
											if (!pluginRequire) {
												console.warn(`[DynamicRouter] 无法获取插件的 webpack runtime require 函数`);
												return;
											}
											...
											for (const moduleId in chunkData) {
												if (typeof chunkData[moduleId] === 'function') {
													...
													if (exports && exports.default) {
														...
														console.log(`[DynamicRouter] 通过插件 webpack runtime 成功加载插件模块（共享机制）`);
														...
														moduleResolved = true;
														window.webpackJsonp.push = originalPushMethod;
														console.log(`[DynamicRouter] 通过插件 webpack runtime 成功加载插件模块（使用第一个导出）`);
											...
											console.error(`[DynamicRouter] 执行模块函数失败:`, e);
										}
									}
								}, 100);
							}
			...
			LazyComp = React.lazy(async () => {
				...
				try {
					...
					if (comp) {
						if (process.env.NODE_ENV === 'development') {
							console.log(`[DynamicRouter] ESM插件加载成功: ${pluginCode}`, {
								version: pluginInfo.version,
								esmUrl: esmUrl,
					...
					// ESM 加载失败，回退到 JSONP 格式
					if (process.env.NODE_ENV === 'development') {
						console.warn(`[DynamicRouter] ESM格式加载失败，回退到JSONP: ${pluginCode}`, esmError);
					}
					...
					try {
						...
						console.warn(`[DynamicRouter] 无法加载 asset-manifest.json，使用默认命名规则:`, e);
					}
					...
					if (typeof __webpack_require__ === 'undefined' || !__webpack_require__.e) {
						console.warn(`[DynamicRouter] webpack runtime 不可用，回退到插件 webpack runtime`);
					...
					try {
							...
							window.webpackJsonp.push = function(...args) {
								...
								if (args.length >= 1 && Array.isArray(args[0]) && args[0].length >= 2) {
									...
									if (Array.isArray(firstArg[0]) && typeof firstArg[1] === 'object') {
										...
										// 调试日志
										if (process.env.NODE_ENV === 'development') {
											console.log('[DynamicRouter] 拦截到 webpackJsonp.push，chunkData:', Object.keys(chunkData));
										}
										...
										for (const moduleId in chunkData) {
											if (typeof chunkData[moduleId] === 'function') {
												try {
													...
													if (!mainAppRequire) {
														console.warn(`[DynamicRouter] 主应用的 webpack runtime 不可用`);
														break;
													}
													...
													if (exports && exports.default) {
														...
														console.log(`[DynamicRouter] 通过主应用 webpack runtime 成功加载插件模块（继承机制）`);
														resolve({ default: exports.default });
														return;
													} else if (exports && typeof exports === 'object' && Object.keys(exports).length > 0) {
														// 如果没有 default，使用第一个导出
														...
														window.webpackJsonp.push = originalPushMethod;
														__webpack_require__.p = originalPublicPath;
														console.log(`[DynamicRouter] 通过主应用 webpack runtime 成功加载插件模块（使用第一个导出）`);
														resolve({ default: exports[firstKey] });
														return;
													}
												} catch (e) {
													console.error(`[DynamicRouter] 执行插件模块函数失败:`, e);
												}
											}
										}
									}
						...
						// 恢复原始的 publicPath
						__webpack_require__.p = originalPublicPath;
						console.warn(`[DynamicRouter] 通过主应用 webpack runtime 加载失败，回退到插件 webpack runtime:`, e);
						return loadPluginChunkWithPluginRuntime(chunkUrl, pluginPublicPath);
					}
				}
			});
