...
import SimpleForm from "@/pages/sys/res/components/SimpleForm";
import {queryLayerCategory} from "@/pages/sys/category/service";
import {queryEnumResource, queryEnumTemplate} from "@/services/enum";
 ...
 * @param fields
 */
const handleSimpleAdd = async (fields) => {
  const hide = message.loading('正在添加');

  try {
    await addSimpleEntity({...fields});
    hide();
    message.success('添加成功');
    return true;
  } catch (error) {
...
const ResourceList = () => {
  ...
  const [createSimpleVisible, handleSimpleVisible] = useState(false);
  ...
  return (
    ...
    >
        ...
        <ProTableX
          ...
          request={async (params, sorter, filter) => {
            const res = await queryPage({
              ...params,
              sorter: {'displayOrder': 'ascend', ...sorter},
              filter: {...filter, "resourceType":["button","menu"]}
            });
      ...
      {createSimpleVisible && <SimpleForm
        onSubmit={async (value) => {
          const success = await handleSimpleAdd(value);

          if (success) {
            handleSimpleVisible(false);
            setParentId('0');
            if (actionRef.current) {
              actionRef.current.reload();
            }
        ...
        modalVisible={createSimpleVisible}
        apps={apps}
        templates={templates}
