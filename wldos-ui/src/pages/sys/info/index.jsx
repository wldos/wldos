...
import {offlineEntity, publishEntity, removeEntities, removeEntity} from "@/pages/sys/article/service";
...
const InfoList = () => {
  ...
  const enumTerms = (tags = [], classType = 'tag') => {
    ...
    for (let i = 0, len = tags.length; i < len; i += 1) {
      const tag = tags[i];
      if (tag)
        temp.push((<Link key={`${tag.id}-art-tag-${i}`} to={`/info/${classType}/${tag.slug}`} target="_blank">{`${tag.name} `}</Link>));
    }
    return temp;
  };
  ...
  const columns = [
    {
      ...
      render: (dom, entity) => {
        return <Link to={`/info-${entity.id}/preview`} target="_blank">{dom}</Link>;
      },
      width: '16%'
    },
    {
      ...
      render: (_, {member,  createBy}) => {
        return member && <Link to={`/info-author/${createBy}.html`} target="_blank">{member.nickname}</Link>;
      },
    },
    {
      title: '分类',
