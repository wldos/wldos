...
import io.github.wldos.platform.support.cms.CMSUtils;
...
public class KCMSService extends NonEntityService {
	...
	public void update(Pub pub, Long userId, String userIp, Long domainId) {
		...
		if (!ObjectUtils.isBlank(pubTypeExt)) {
			...
			this.pubmetaService.saveOrUpdateAll(pubMetasN, SaveOptions.forImport());
			if (!pubMetasU.isEmpty())
				this.pubmetaService.saveOrUpdateAll(pubMetasU, SaveOptions.forImport());
		}
	...
	public MainPicture exact(List<KPubmeta> metas, String pic) {
		...
		for (KPubmeta meta : metas) {
			if (meta.getMetaKey().equals(pic)) {
				mp.setUrl(this.store.getPublicUrl(meta.getMetaValue(), null));
				break;
			}
		}
		if (mp.getUrl() == null) {
	...
	void createPubMeta(List<PubTypeExt> pubTypeExt, Long id) {
		...
		this.pubmetaService.saveOrUpdateAll(pubMetas, SaveOptions.forImport());
	}
